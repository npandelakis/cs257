...
def filter_books(filtered, books) -> list:
    title_index = 0
    return list(filter(lambda f: any(substring.lower() in f[title_index].lower() for substring in books), filtered))


def filter_authors(filtered, authors) -> list: 
    author_index = 2
    return list(filter(lambda f: any(substring.lower() in f[author_index].lower() for substring in authors), filtered))


def filter_years(filtered, year1, year2) -> list:
    year_index = 1
    return list(filter(lambda f: year1 <= f[year_index] and year2 >= f[year_index], filtered))
